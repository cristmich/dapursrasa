    ...
    return (
      <div className="bg-[#005926]/5 border border-[#005926]/20 rounded-2xl p-8 text-center flex flex-col items-center justify-center min-h-[400px]">
        ...
        <p className="text-gray-600 mb-6">
          Anda akan dialihkan ke WhatsApp untuk melanjutkan percakapan dengan
          admin kami.
        </p>
        <Button
          onClick={() => setIsSuccess(false)}
          variant="outline"
          className="rounded-full border-[#005926] text-[#005926]"
        >
          Kirim Pesan Lainnya
        </Button>
      </div>
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          <Label htmlFor="nama" className="text-[#333333]">
            Nama Lengkap *
          </Label>
          <Input
            id="nama"
            placeholder="John Doe"
            className="rounded-lg bg-[#F8F8F8] border-gray-200 focus-visible:ring-[#005926]"
            {...register("nama")}
          />
          {errors.nama && (
        ...
        <div className="space-y-2">
          <Label htmlFor="noHp" className="text-[#333333]">
            Nomor WhatsApp *
          </Label>
          <Input
            id="noHp"
          ...
          />
          {errors.noHp && (
            <p className="text-sm text-red-500">{errors.noHp.message}</p>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          <Label htmlFor="jenisAcara" className="text-[#333333]">
            Jenis Acara *
          </Label>
          <Input
            id="jenisAcara"
            placeholder="Contoh: Wedding, Seminar, dll"
            className="rounded-lg bg-[#F8F8F8] border-gray-200 focus-visible:ring-[#005926]"
            {...register("jenisAcara")}
          />
          {errors.jenisAcara && (
            <p className="text-sm text-red-500">{errors.jenisAcara.message}</p>
          )}
        </div>
        <div className="space-y-2">
          <Label htmlFor="jumlahPax" className="text-[#333333]">
            Jumlah Porsi (Pax) *
          </Label>
          <Input
            id="jumlahPax"
          ...
          />
          {errors.jumlahPax && (
            <p className="text-sm text-red-500">{errors.jumlahPax.message}</p>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          <Label htmlFor="tanggal" className="text-[#333333]">
            Tanggal Acara *
          </Label>
          <Input
            id="tanggal"
        ...
        <div className="space-y-2">
          <Label htmlFor="lokasi" className="text-[#333333]">
            Lokasi Acara *
          </Label>
          <Input
            id="lokasi"
            placeholder="Detail alamat atau gedung"
            className="rounded-lg bg-[#F8F8F8] border-gray-200 focus-visible:ring-[#005926]"
            {...register("lokasi")}
          />
          {errors.lokasi && (
      ...
      <div className="space-y-2">
        <Label htmlFor="pesan" className="text-[#333333]">
          Catatan Tambahan
        </Label>
        <Textarea
          id="pesan"
        ...
        />
        {errors.pesan && (
          <p className="text-sm text-red-500">{errors.pesan.message}</p>
        )}
      </div>
