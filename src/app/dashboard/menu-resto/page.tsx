...
export default function MenuRestoAdmin() {
  ...
  const handleOpenAddForm = () => {
    ...
    const prefilled = activeMenus.map((m) => {
      ...
      return {
        ...
        price: existingRestoItem
          ? existingRestoItem.price.toString()
          : m.price.toString(),
        category: existingRestoItem
          ? existingRestoItem.category
          : m.category || "Makanan Utama",
        stockIn: 0,
      };
    });
  ...
  const handleTutupTokoSubmit = async () => {
    ...
    try {
      await Promise.all([
        ...
        ...menus.map((m) =>
          updateMenu("menus_resto", m.id, { stockOut: m.stockIn || 0 }),
        ),
        statusDocId
          ? updateMenu("resto_status", statusDocId, {
              isShopOpen: false,
              lastClosedDate: todayStr,
            })
          : addMenu("resto_status", {
              isShopOpen: false,
              lastClosedDate: todayStr,
            }),
      ]);
  ...
  const handleBukaToko = async () => {
    ...
    if (lastClosedDate === todayStr) {
      alert(
        "Toko sudah ditutup untuk hari ini! Anda baru bisa membuka toko kembali besok hari.",
      );
      return;
    }

    const wantToOpen = confirm(
      "Buka toko untuk hari ini?\n\nStok dari hari sebelumnya akan direset menjadi 0 untuk memulai hari yang baru.",
    );
    ...
    try {
      ...
      menus.forEach((m) => {
        promises.push(
          updateMenu("menus_resto", m.id, { stockIn: 0, stockOut: 0 }),
        );
      });

      if (statusDocId) {
        promises.push(
          updateMenu("resto_status", statusDocId, { isShopOpen: true }),
        );
      } else {
  ...
  const handleAddMenu = async () => {
    for (let i = 0; i < newMenus.length; i++) {
      if (!newMenus[i].name.trim() || !newMenus[i].price.toString().trim()) {
        return alert(
          `Baris ke-${i + 1} belum lengkap! Pastikan Menu (Nama) dan Harga terisi.`,
        );
      }
    }
    ...
    try {
      await Promise.all(
        newMenus.map((menu) => {
          ...
          const existingRestoItem = menus.find(
            (rm) => rm.daftarMenuId === menu.daftarMenuId,
          );
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Kelola daftar menu untuk Dine In / Resto serta pelacakan stok porsi.
          </p>
        </div>
          ...
          <div className="bg-white w-full max-w-5xl rounded-2xl shadow-xl flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50 rounded-t-2xl">
              <h2 className="text-xl font-bold text-gray-900">
                Tambah Menu Resto (Dari Daftar Menu)
              </h2>
              <Button
                variant="ghost"
                size="icon"
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-4">
              <p className="text-sm text-gray-500 mb-4">
                Berikut adalah semua menu yang aktif di toko. Silakan input stok
                masuknya.{" "}
                <b>
                  Jika menu sudah ada di tabel, stok barunya akan ditambahkan
                  (diakumulasi) ke stok lama.
                </b>{" "}
                Biarkan 0 jika belum ada stok tambahan.
              </p>

              {newMenus.map((menu, index) => (
                <div
                  key={index}
                  className="flex flex-col lg:flex-row gap-3 items-start lg:items-center bg-gray-50 p-4 rounded-xl border border-gray-100"
                >
                  ...
                  <div className="w-full lg:w-32 space-y-1.5">
                    ...
                    <Input
                      ...
                      value={menu.stockIn}
                      onChange={(e) =>
                        handleAddChange(index, "stockIn", e.target.value)
                      }
          ...
          <div className="bg-white w-full max-w-md rounded-2xl shadow-xl flex flex-col">
            ...
            <form onSubmit={handleUpdateMenu} className="p-6 space-y-4">
              ...
              <p className="text-xs text-gray-500 mb-2">
                Input angka di bawah untuk <b>menambah</b> jumlah stok (bukan
                mengganti totalnya). Biarkan 0 jika tidak ada penambahan.
              </p>
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between items-end">
                    ...
                    <span className="text-[10px] text-gray-400">
                      Total skrg: {editData.currentStockIn}
                    </span>
                  </div>
                  <Input
                    type="number"
                    ...
                    onChange={(e) =>
                      setEditData({
                        ...editData,
                        addStockIn: Number(e.target.value),
                      })
                    }
                  ...
                  <p className="text-[10px] text-green-600/80">
                    Nanti menjadi:{" "}
                    {editData.currentStockIn +
                      (Number(editData.addStockIn) || 0)}
                  </p>
                </div>
                <div className="space-y-1.5">
                  <div className="flex justify-between items-end">
                    ...
                    <span className="text-[10px] text-gray-400">
                      Total skrg: {editData.currentStockOut}
                    </span>
                  </div>
                  <Input
                    type="number"
                    ...
                    onChange={(e) =>
                      setEditData({
                        ...editData,
                        addStockOut: Number(e.target.value),
                      })
                    }
                  ...
                  <p className="text-[10px] text-red-600/80">
                    Nanti menjadi:{" "}
                    {editData.currentStockOut +
                      (Number(editData.addStockOut) || 0)}
                  </p>
                </div>
              </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-gray-500 mt-1">
              {selectedCategory === "Semua"
                ? "Tambahkan item menu resto dari Daftar Menu."
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50/80 border-b border-gray-100 text-gray-500 text-xs uppercase tracking-wider">
                  ...
                  <th className="p-4 font-semibold whitespace-nowrap text-center text-[#005926]">
                    Sisa Stok
                  </th>
                </tr>
              </thead>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-4 text-center">
                        <span
                          className={`inline-flex items-center justify-center min-w-[32px] h-8 rounded-lg text-sm font-bold ${sisaStok > 10 ? "bg-green-100 text-green-700" : sisaStok > 0 ? "bg-yellow-100 text-yellow-700" : "bg-red-100 text-red-700"}`}
                        >
                          {sisaStok}
                        </span>
                      </td>
